/**
 * EFF Short Wordlist #1 — source data for passphrase generation.
 *
 * Source:    Electronic Frontier Foundation, "Deep Dive: EFF's New Wordlists
 *            for Random Passphrases" (2016).
 *            https://www.eff.org/deeplinks/2016/07/new-wordlists-random-passphrases
 * File:      eff_short_wordlist_1.txt
 * License:   Released by EFF with no claimed copyright (public-domain-equivalent;
 *            EFF explicitly intends this list to be redistributed in software).
 * Original size: 1296 words (= 6⁴, sized for four six-sided dice rolls).
 *
 * Deliberately kept in source control, not a secret store (DB/env). See the
 * "Why not hide this?" section in password-generator.ts — the security of a
 * generated passphrase comes from its length (entropy), not from concealing
 * a well-known public wordlist.
 *
 * One entry — "yo-yo" — is excluded below. The original list contains both
 * "yo-yo" and "yoyo" as two distinct words; "yo-yo"'s embedded hyphen would
 * collide with the "-" separator generateReadablePassword() uses to join
 * words, silently turning what should be N words into N+1 dash-delimited
 * segments. Dropping one entry out of 1296 changes the per-word entropy by
 * a practically unmeasurable amount (see password-generator.ts).
 */
